				...
				<a class="thumbnail" href="{{ tenantRoute('proposition', [$proposition['id']]) }}">
		         	<div class="caption">
			          	<p>@if (empty($proposition['marker']) == false) 
		       			...
		       			<small class="text-muted" style="margin-top: 10px; display: block;"><img class="img-circle text-sized-picture" src="{{ $proposition['proposer']['avatar'] }}"> {{ $proposition['proposer']['displayName'] }}</small>
		       		</div>
	       		</a>
        		@endforeach
